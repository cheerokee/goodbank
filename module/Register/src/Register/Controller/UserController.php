...
class UserController extends CrudController
{
    ...
    public function editAction($request = null)
    {
        ...
        if($request->isPost()){
            ...
            if(isset($data_arr['document']) && $data_arr['document'] != '')
            {
                ...
                if($has_user && $has_user->getId() != $data_arr['id']){
                    ...
                    {
                        header('Location: ' . $data_arr['redirect']);
                        die;
                    }else{
        ...
        $view = parent::editAction($request,null,$redirect);
        return $view;// TODO: Change the autogenerated stub
    }
    ...
    public function newAction($request = null)
    {
        ...
        if($request->isPost()){
            ...
            if(isset($data_arr['reference']) && $data_arr['reference'] == 'direct'){
                /** Enviado pelo formulário administrativo ***/
                $data_arr['active'] = 1;
            }
        ...
        $view = parent::newAction($request,null,$redirect); // TODO: Change the autogenerated stub
        return $view;
    }

    public function attrRoleAction(){
        $em = $this->getEm();
        $db_users = $em->getRepository('Register\Entity\User')->findAll();
        if(!empty($db_users)){
            foreach($db_users as $db_user)
            {
                $db_user_roles = $em->getRepository('Register\Entity\UserRole')->findByUser($db_user);

                if(empty($db_user_roles)){
                    $db_role = $em->getRepository('Acl\Entity\Role')->findOneByName('Cliente');
                    if($db_role){
                        /**
                         * @var UserRole $db_user_role
                         */
                        $db_user_role = new UserRole();
                        $db_user_role->setUser($db_user);
                        $db_user_role->setRole($db_role);

                        $em->persist($db_user_role);
                        $em->flush();
                    }
                }
            }
    ...
    public function rapidUserAction(){
